...
if __name__ == '__main__':
    ...
    with open(args.prediction_path, mode="w", encoding="utf-8") as f:
        ...
        for k in range(len(src_tensor)):
            ...
            for i in range(args.seq_length - beginning_length[k]):
                output = model(target_temp.to(device), seg_temp.to(device))
                output = output.cpu()
            ...
            generated_sentence = "".join(
                args.tokenizer.convert_ids_to_tokens([token_id.item() for token_id in src_tensor_list[k][0]])
            )
            f.write(generated_sentence + '\n')
            end_time = time.time()
            print(f"time consumption this loop:{end_time - star_time} seconds")
