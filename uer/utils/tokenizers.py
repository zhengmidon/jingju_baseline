...
class CharTokenizer(Tokenizer):
    ...
    def __init__(self, args, is_src=True):
        ...
        self.basic = BasicTokenizer(False)

    def tokenize(self, text, use_vocab=True):
        text = convert_to_unicode(text)
        text = self.basic._clean_text(text)

        text = self.basic._tokenize_chinese_chars(text)

        orig_tokens = whitespace_tokenize(text)
        if use_vocab:
            return [token if token in self.vocab else UNK_TOKEN for token in orig_tokens]
        else:
...
class BertTokenizer(Tokenizer):
    ...
    def __init__(self, args, is_src=True, do_lower_case=False):
        super().__init__(args, is_src)
