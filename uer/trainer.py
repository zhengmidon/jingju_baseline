    ...
    def train(self, args, gpu_id, rank, loader, model, optimizer, scheduler):
        ...
        while True:
            ...
            if self.current_step % self.report_steps == 0 and \
                ...
                save_label = self.validate(args,model,gpu_id)
                if save_label:
                    save_model(model, self.output_model_path + "-" + str(self.current_step))
                    print(f'Early stop at {self.current_step} step')
                    break
                self.start_time = time.time()
...
class MlmTrainer(Trainer):
    def __init__(self, args):
        ...
        self.validation_loss = 100000.
        self.validation_retry = 0
    ...
    def forward_propagation(self, batch, model):
        ...
        #average the loss across accumulation steps
    ...
    def validate(self,args,model,gpu_id):
        print('------validating------')
        begin = time.time()
        model.eval()
        devset_reader = open(args.devset_path, "rb")
        total_loss = 0.
        n_batches = 0
        while True:
            instances = []
            try:
                for i in range(args.batch_size):
                    instances.append(pickle.load(devset_reader))
                ...
                src = torch.LongTensor(src).cuda(gpu_id)
                tgt = torch.LongTensor(tgt).cuda(gpu_id)
                seg = torch.LongTensor(seg).cuda(gpu_id)
                with torch.no_grad():
                    loss_info = model(src, tgt, seg)
                loss, correct, denominator = loss_info
        ...
        if avg_loss < self.validation_loss:
            self.validation_loss = avg_loss
            self.validation_retry = 0
            save_label = False
        else:
            self.validation_retry += 1
            if self.validation_retry > 5:
                save_label = True
            else:
                save_label = False
        end = time.time()
